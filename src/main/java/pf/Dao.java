	...
	public static String query(final String q, final Predicate<Node> filter) {
		try (final Transaction ignored = DB.beginTx(); final Result result = DB.execute(q)) {
			...
			relationships.stream()
					.filter(r -> graph.nodes.containsKey(r.getStartNode().getId())
							&& graph.nodes.containsKey(r.getEndNode().getId()))
					.filter(rel -> getLabel(rel.getEndNode()).equals("ARISTA")).forEach(rel -> {
			...
			relationships.stream()
					.filter(r -> graph.nodes.containsKey(r.getStartNode().getId())
							&& graph.nodes.containsKey(r.getEndNode().getId()))
					.filter(rel -> getLabel(rel.getEndNode()).equals("VALOR")).forEach(rel -> {
			...
			relationships.stream()
					.filter(r -> nodePositions.containsKey(r.getStartNode().getId())
							&& nodePositions.containsKey(r.getEndNode().getId()))
					.forEach(rel -> links.add(new Link(nodePositions.get(rel.getStartNode().getId()),
							nodePositions.get(rel.getEndNode().getId()))));
